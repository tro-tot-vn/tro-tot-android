...
| 2 | `latex/chapter2_auth/chapter2.tex` | Thành viên 2 | File chapter khác |
| 3 | `latex/chapter3_profile/chapter3.tex` | Thành viên 2 | File chapter khác |
| 4 | `latex/chapter4_post_search/chapter4.tex` | Thành viên 3 | File chapter khác |
| 5 | `latex/chapter5_chat_videocall/chapter5.tex` | Thành viên 4 | File chapter khác |
| 6 | `latex/chapter6_admin/chapter6.tex` | Trưởng nhóm | File chapter khác |
...
## 2. Khung bắt buộc cho mỗi `\section` (Chương 2–5)
...
- **Bắt buộc:** ít nhất **1 hình screenshot** màn hình (quy tắc đặt tên + chất lượng: xem mục 3.1).
...
- Có thể trích **đoạn code ngắn** — tuân theo quy tắc ở mục 3.2, không paste cả file.

---

## 3. Minh họa trong báo cáo: hình ảnh và code

Nguyên tắc chung: minh họa để **chứng minh**, không phải để **độn trang**. Báo cáo chấm theo CLO, không chấm theo số trang.

### 3.1. Hình ảnh và sơ đồ

| Loại | Đặt tại | Quy tắc đặt tên |
|------|---------|-----------------|
| Screenshot UI | `latex/images/outputs/` | `c2_login_screen.png`, `c4_home_feed.png`, … |
| Sơ đồ Mermaid (nguồn) | `latex/images/` | `c5_chat_flow.mmd` |
...
- Tiền tố `c1_` … `c6_` **đúng với số chương** — tránh ghi đè ảnh người khác.
- Mỗi chương (2–5): tối thiểu **2 hình** (screenshot hoặc sơ đồ).
- Screenshot resize về **~1080px chiều ngang** trước khi commit (ảnh gốc 1440p+ nặng gấp 3–4 lần, in ra không khác gì).
- Không chụp màn hình mờ, dính dữ liệu cá nhân thật, hoặc trạng thái lỗi ngoài ý muốn.
- Mỗi hình phải có **caption tiếng Việt** và được **nhắc đến trong text** (qua `\ref{fig:...}` hoặc mô tả) — không thả hình mồ côi.

### 3.2. Trích code

Dùng `\begin{lstlisting}...\end{lstlisting}` (preamble đã cấu hình style Java sẵn — **không** tự chỉnh `\lstset`).

**Liều lượng (bắt buộc):**

- Mỗi `\section` lớn: **tối đa 1 đoạn code**. Cả chương: **tối đa 4 đoạn**.
- Mỗi đoạn: **5–15 dòng**. Quá 15 dòng → cắt bớt hoặc tách ý, không co font.

**Chỉ paste code khi nó chứng minh được kỹ thuật.** Ví dụ đáng paste:

- Logic refresh token khi gặp 401 (`TokenAuthenticator`).
- Khởi tạo `EncryptedSharedPreferences` (AES256).
- Validation regex trong ViewModel.
- Xử lý phân trang con trỏ / append dữ liệu.

**Cấm:**

- Paste cả file, cả class, cả Fragment/Activity.
- Layout XML dài (đã có screenshot thay vai trò minh họa giao diện).
- Getter/setter, import, boilerplate, code sinh tự động.
- Thả code trần: **mọi đoạn code phải có 1–2 câu dẫn** trước nó — nêu rõ đoạn này làm gì và vì sao đáng xem. Không viết nổi câu dẫn nghĩa là đoạn code đó không đáng nằm trong báo cáo.
- Giữ comment tiếng Anh nguyên văn trong code trích — xóa hoặc dịch.

**Trước khi nộp:** build lại PDF, kiểm tra đoạn code **không tràn lề** (dòng quá dài → reword hoặc xuống dòng thủ công).
...
[ ] ≥ 2 hình ảnh, đặt tên đúng c{N}_*, có caption + được nhắc trong text
[ ] Code trích ≤ 4 đoạn/chương, mỗi đoạn ≤ 15 dòng, có câu dẫn
...
**Phiên bản:** 1.1 — áp dụng đồ án Trọ Tốt Android, học phần PTUD thiết bị di động.
